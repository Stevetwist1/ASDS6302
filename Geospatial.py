...
# Precompute values and figures
status_values   = sorted(df['status'].dropna().unique())
borough_values  = sorted(df['borough'].dropna().unique())
borough_options = [{'label': b, 'value': b} for b in borough_values]
...
map_layout = dbc.Container(fluid=True, children=[
    ...
    dbc.Row([
        dbc.Col([
            html.Label('Health', className='text-white me-2'),
            dcc.Checklist(
                id='health-check',
        ...
        dbc.Col([
            html.Label('Borough', className='text-white me-2'),
            dcc.Dropdown(
                id='borough-dropdown',
                options=borough_options,
                value=borough_values,
                multi=True,
    ...
    dcc.Graph(id='main-map', style={'width':'100%','height':'75vh'}),
    html.H3('Map Data Table', className='text-white mt-4'),
    dash_table.DataTable(
        id='map-table',
        columns=[{'name': col.replace('_',' ').title(), 'id': col}
                 for col in ['tree_id','address','spc_common','health','status','council district']],
        page_size=10,
...
def update_map(sel_h, sel_b):
    d = df[df['health'].isin(sel_h) & df['borough'].isin(sel_b)]
    agg = d.groupby('council district').agg(
        count=('tree_id','count'),
    ...
    fig = px.scatter_mapbox(
        ...
        hover_data={'count':True,'council district':True},
...
def update_analytics_map(sel_status):
    ...
    center = {'lat': agg['latitude'].mean(), 'lon': agg['longitude'].mean()}
    color_scale = {
        'Alive': ['lightgreen','green'],
        'Fair' : ['lightblue','blue'],
        'Dead' : ['lightcoral','red']
    }.get(sel_status, ['lightblue','darkblue'])
    fig = px.scatter_mapbox(
        agg, lat='latitude', lon='longitude',
        size='count', color='count',
    ...
    fig.update_layout(
        mapbox_style='open-street-map',
        paper_bgcolor='#343a40', plot_bgcolor='#343a40',
        coloraxis_colorbar=dict(title='Count', tickfont=dict(color='white'))
    )
    return fig
